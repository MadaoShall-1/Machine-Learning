...
class EnhancedStreamingDataset(Dataset):
    """
    Sequence-labeling dataset: each item is a full sentence/chunk with per-token
    switch and duration labels.  This gives the model O(seq_len) supervised
    signal per forward pass instead of 1, which is the single biggest driver
    of low F1 in the previous point-prediction formulation.
    """
    ...
    def __init__(
        ...
        # Build one dataset item per sentence (chunk to max_context_window)
        all_sequences = []
        for sample in raw_samples:
            ...
            # Slide a window of max_context_window over the sentence
            # so very long sentences are not truncated entirely.
            n = len(token_ids)
            step = max(1, max_context_window // 2)   # 50 % overlap
            for start in range(0, n, step):
                end = min(start + max_context_window, n)
                chunk_ids   = token_ids[start:end]
                chunk_sw    = switch_labels[start:end]
                chunk_dur   = duration_labels[start:end]

                # Skip chunks with no valid labels at all
                if all(l == -1 for l in chunk_sw):
                    continue

                all_sequences.append({
                    'token_ids':       chunk_ids,
                    'switch_labels':   chunk_sw,
                    'duration_labels': chunk_dur,
                    'pair':            pair,
                })

        # Optional cap — stratify by pair to keep language balance
        if max_samples is not None and len(all_sequences) > max_samples:
            import random
            random.seed(42)
            strata = defaultdict(list)
            for seq in all_sequences:
                strata[seq['pair']].append(seq)
            total = len(all_sequences)
            sampled = []
            for pair_seqs in strata.values():
                n_take = max(1, int(max_samples * len(pair_seqs) / total))
                sampled.extend(random.sample(pair_seqs, min(n_take, len(pair_seqs))))
            random.shuffle(sampled)
            self.sequences = sampled
        else:
            self.sequences = all_sequences

        logger.info(f"Sequence-labeling dataset: {len(self.sequences)} chunks "
                    f"(max_len={max_context_window})")

        pair_counts = defaultdict(int)
        for s in self.sequences:
            pair_counts[s['pair']] += 1
        for pair, cnt in sorted(pair_counts.items()):
            logger.info(f"  {pair}: {cnt} chunks")
    ...
    def __getitem__(self, idx):
        ...
        switch_labels   = seq['switch_labels']
        duration_labels = seq['duration_labels']

        length = len(token_ids)
        pad_len = self.max_context_window - length

        # Right-pad with zeros
        input_ids      = token_ids       + [0] * pad_len
        sw_labels_pad  = switch_labels   + [-1] * pad_len
        dur_labels_pad = duration_labels + [-1] * pad_len
        attn_mask      = [1] * length    + [0] * pad_len

        result = {
            'input_ids':       torch.tensor(input_ids,      dtype=torch.long),
            'attention_mask':  torch.tensor(attn_mask,      dtype=torch.long),
            'switch_labels':   torch.tensor(sw_labels_pad,  dtype=torch.long),
            'duration_labels': torch.tensor(dur_labels_pad, dtype=torch.long),
        }

        if self.include_pair_info:
            result['pair'] = seq['pair']
...
class CodeSwitchTrainer:
    ...
    def __init__(
        ...
        # The pretrained encoder needs gentle updates (1/10th of the head LR)
        # to avoid catastrophic forgetting of its multilingual representations.
        # The fresh classification heads can use the full learning rate.
        encoder_params = list(model.encoder.parameters()) + list(model.layer_norm.parameters())
        head_params    = list(model.switch_head.parameters()) + list(model.duration_head.parameters())
        ...
        logger.info(f"Layer-wise LR — encoder: {config.learning_rate * 0.1:.2e}, heads: {config.learning_rate:.2e}")
    ...
    def _collate_fn(self, batch):
        """Custom collate function for sequence-labeling batches"""
        input_ids      = torch.stack([item['input_ids']       for item in batch])
        attention_mask = torch.stack([item['attention_mask']  for item in batch])
        # New dataset already outputs 'switch_labels' / 'duration_labels' as tensors
        switch_labels   = torch.stack([item['switch_labels']   for item in batch])
        duration_labels = torch.stack([item['duration_labels'] for item in batch])
    ...
    def _calculate_class_weights(
        ...
        """Calculate inverse frequency class weights from sequence-label dataset"""
        label_key = 'switch_labels' if task == 'switch' else 'duration_labels'
        ...
        for item in dataset:
            ...
            # labels is a 1-D tensor of length max_context_window
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Forward pass — bidirectional attention (no causal mask) for offline task
            outputs = self.model(
                input_ids=input_ids,
                attention_mask=attention_mask,
            ...
            # Use ALL sequence positions — full supervised signal
            # switch_logits:   [batch, seq_len, 2]
            # duration_logits: [batch, seq_len, num_classes]
            # Labels:          [batch, seq_len]  (-1 positions are ignored by loss)
            switch_logits   = outputs['switch_logits']    # [batch, seq_len, 2]
            duration_logits = outputs['duration_logits']  # [batch, seq_len, num_classes]
    ...
    def evaluate(self, data_loader: DataLoader, split_name: str = 'val') -> Dict[str, float]:
        ...
        for batch in tqdm(data_loader, desc=f"Evaluating {split_name}"):
            ...
            # Compute loss over all valid positions
            losses = self.criterion(
                switch_logits=switch_logits,
                duration_logits=duration_logits,
                switch_labels=switch_labels,
            ...
            # Update metrics — pass full sequences directly
            if pairs:
...
class StreamingPredictor:
    ...
    def predict_next(self, text: str) -> Dict:
        ...
        # Forward pass — bidirectional (consistent with training/eval)
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            apply_causal_mask=False
...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=0.4,
                        help='Classification threshold for switch prediction (default 0.4)')
    parser.add_argument('--loss_type', type=str, default='focal',
                        choices=['ce', 'focal'],
                        help='Loss function type: "ce" for Cross-Entropy, "focal" for Focal Loss (default)')
