...
class LanguageDetector:
    ...
    def __init__(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                batch_size = 32
                print(f"   🚀 Using GPU for language detection (CUDA)")
            else:
                device = -1
                batch_size = 8
                print(f"   💻 Using CPU for language detection")
    ...
    def detect_token_language(self, token_text: str, full_sentence: str, 
        ...
        if pair == 'spanish_eng' and sentence_pattern is not None:
            ...
            # Clean token text (remove tokenizer prefix and punctuation)
            # XLM-R uses '▁' prefix, mBERT uses '##' prefix
    ...
    def tokenize_with_language_ids(self, sentence_meta: Dict) -> Dict:
        ...
        for i, (token, (start, end), is_special) in enumerate(
            ...
            if is_special:
                ...
                # XLM-R (SentencePiece): word starts with '▁' (U+2581)
                # mBERT (WordPiece): continuation tokens start with '##'
                if 'xlm-roberta' in self.config.model_name:
                    is_word_start = token.startswith('▁') or (i == 0)
                else:
                    # mBERT: tokens WITHOUT '##' prefix are word starts
